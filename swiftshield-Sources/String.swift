...
extension String {
    ...
    func injectTypeAliases(classes: [String: (SwiftScope,String)]) -> String {
        return classes.reduce("", { (result, data: (key: String, value: (SwiftScope, String))) -> String in
            return result + "\(data.value.0.rawValue) typealias \(data.key) = \(data.value.1)\n"
        }) + self
    }
}
